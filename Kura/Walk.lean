      ...
      · obtain ⟨v, hv, hv2⟩ := h
        ...
        · have hexists : ∃ u ∈ (cons y e' w).vx, P u := by use v
          ...
          · refine endIf_dropLast_mem_vx (w := cons y e' w) (by use v) hnonempty ?_
            by_cases hPy : P y
            · simp only [endIf_cons, hPy, ↓reduceDIte]
              sorry
            ·
              -- simp only [endIf_cons, hPx, ↓reduceDIte] at hu
              -- rw [dropLast_vx, cons_vx, List.dropLast_cons] at hu
              -- simp_all
              sorry
          · simp only [Nonempty.not_iff] at hnonempty
...
lemma IsBetween.path_vx (hbtw : G.IsBetween e u v) (hne : u ≠ v) :
  ...
  simp only [Path.vx, path, Walk.vx]
...
instance IsVxSetSeparator.IsPreorder {S : Set α} :
    IsPreorder (Set α) (IsVxSetSeparator G · S ·) where
  refl A := by
    rintro s hs a ha hconn
    have := hconn.mem_right
    simp only [induce_V, mem_diff, ha, not_true_eq_false, and_false] at this
  trans A B C hAB hBC := by
    rintro s hs c hc hconn
    rw [Connected.iff_path] at hconn
    obtain ⟨p, hpVd, rfl, rfl⟩ := hconn
    obtain ⟨x, hxp, hxB⟩ := hBC.path_inter p (hpVd.le (induce_le G Set.diff_subset)) hs hc
    have := p.val.connected_of_validOn_of_mem hpVd start_vx_mem hxp
    exact hAB p.val.start hs x hxB this

lemma IsVxSetSeparator.crossingWalk_intersect {V S T : Set α} (h : G.IsVxSetSeparator V S T)
    [DecidablePred (· ∈ V)] {w : Walk α β} (hwVd : w.ValidOn G) (hwstart : w.start ∈ h.leftSet ∪ V)
    (hwfinish : w.finish ∈ h.rightSet ∪ V) : ∃ x ∈ w.vx, x ∈ V:= by
  sorry

lemma IsVxSetSeparator.crossingWalk_intersect' {V S T : Set α} (h : G.IsVxSetSeparator V S T)
    [DecidablePred (· ∈ V)] {w : Walk α β} (hwVd : w.ValidOn G) (hwstart : w.start ∈ h.rightSet ∪ V)
    (hwfinish : w.finish ∈ h.leftSet ∪ V) : ∃ x ∈ w.vx, x ∈ V:= by
  sorry

lemma IsVxSetSeparator.crossingWalk_endIf_validOn {V S T : Set α} (h : G.IsVxSetSeparator V S T)
    [DecidablePred (· ∈ V)] (w : Walk α β) (hwVd : w.ValidOn G) (hwstart : w.start ∈ h.leftSet ∪ V)
    (hwfinish : w.finish ∈ h.rightSet ∪ V) : (w.endIf (P := (· ∈ V))
    (h.crossingWalk_intersect hwVd hwstart hwfinish)).ValidOn (G[h.leftSet ∪ V]) := by
  sorry

lemma IsVxSetSeparator.crossingWalk_endIf_validOn' {V S T : Set α} (h : G.IsVxSetSeparator V S T)
    [DecidablePred (· ∈ V)] (w : Walk α β) (hwVd : w.ValidOn G) (hwstart : w.start ∈ h.rightSet ∪ V)
    (hwfinish : w.finish ∈ h.leftSet ∪ V) : (w.endIf (P := (· ∈ V))
    (h.crossingWalk_intersect' hwVd hwstart hwfinish)).ValidOn (G[h.rightSet ∪ V]) := by
  sorry

lemma IsVxSetSeparator.leftSetV_iff {V S T : Set α} (h : G.IsVxSetSeparator V S T) (hV : V ⊆ G.V)
    (U : Set α) : G[h.leftSet ∪ V].IsVxSetSeparator U S V ↔ G.IsVxSetSeparator U S V := by
  classical
  constructor
  · rintro hUsep s hs v hv hconn
    ...
    have := h.crossingWalk_endIf_validOn w hwVdG hwstart hwfinish
  ...
  · rintro hUsep
    refine hUsep.le ?_
    refine induce_le _ ?_
    refine Set.union_subset ?_ ?_
    · exact leftSet_subset h
    · exact hV

lemma IsVxSetSeparator.rightSetV_iff {V S T : Set α} (h : G.IsVxSetSeparator V S T) (hV : V ⊆ G.V)
    (U : Set α) : G[h.rightSet ∪ V].IsVxSetSeparator U V T ↔ G.IsVxSetSeparator U V T := by
  have := h.symm.leftSetV_iff hV U
  rw [comm (S := V), comm (S := V)]
  convert this using 1

lemma IsVxSetSeparator.conn_sep_iff_conn_left {V S T : Set α} (h : G.IsVxSetSeparator V S T)
    (hu : u ∈ h.leftSet) (hV : V ⊆ G.V) :
    (∃ v ∈ V, G.Connected u v) ↔ ∃ v ∈ V, G[h.leftSet ∪ V].Connected u v := by
  classical
  constructor
  · rintro ⟨v, hv, hconn⟩
    rw [Connected.iff_walk] at hconn
    ...
    · by_cases hxw'Finish : x = w'.finish
      ...
      · have hw'dVdG : w'.dropLast.ValidOn G := dropLast_validOn hw'VdG
        ...
        have : w'.dropLast.ValidOn (G[h.leftSet]) := by
          refine ValidOn.induce hw'dVdG ?_
          rintro y hy
  ...
  · rintro ⟨v, hv, hconn⟩
    ...
    exact union_subset (h.leftSet_subset) hV
...
def insert (p : Path α β) (Ps : PathEnsemble α β) (h : ∀ v ∈ p.vx, v ∉ Ps.VxSet) : PathEnsemble α β where
  Paths := Ps.Paths ∪ {p}
  Disj := by
...
@[simp]
lemma Empty_validOn {G : Graph α β} : (Empty α β).ValidOn G := by
  rintro p hp
  exact False.elim hp
...
@[simp]
lemma nil_validOn {U : Set α} {G : Graph α β} (hUV : U ⊆ G.V) : (nil U β).ValidOn G := by
  rintro p hp
  simp only [nil, ValidOn, mem_image] at hp
  obtain ⟨q, hq, rfl⟩ := hp
  exact hUV hq

@[simp]
lemma mem_nil_iff {U : Set α} {p : Path α β} : p ∈ (nil U β).Paths ↔ ∃ u ∈ U, Path.nil u = p := by
  simp only [nil, mem_image]

@[simp]
lemma nil_VxSet {U : Set α} : (nil U β).VxSet = U := by
...
@[simp]
lemma nil_startSet {U : Set α} : (nil U β).StartSet = U := by
  sorry

@[simp]
lemma nil_finishSet {U : Set α} : (nil U β).FinishSet = U := by
  sorry

@[simp]
lemma nil_ncard {U : Set α} : (nil U β).Paths.ncard = U.ncard :=
...
lemma startSet_subset_VxSet {Ps : PathEnsemble α β} :
    Ps.StartSet ⊆ Ps.VxSet := by
  rintro x ⟨p, hp, rfl⟩
  use p, hp, start_vx_mem

lemma finishSet_subset_VxSet {Ps : PathEnsemble α β} :
    Ps.FinishSet ⊆ Ps.VxSet := by
  rintro x ⟨p, hp, rfl⟩
  use p, hp, finish_vx_mem

lemma VxSet_subset_of_validOn {Ps : PathEnsemble α β} (hVd : Ps.ValidOn G) : Ps.VxSet ⊆ G.V := by
  rintro x ⟨p, hp, hx⟩
...
lemma StartSet_ncard {Ps : PathEnsemble α β} : Ps.StartSet.ncard = Ps.Paths.ncard := by
  rw [eq_comm]
  apply Set.ncard_congr (fun p hp ↦ Path.start p)
  · rintro p hp
    use p
  · rintro p q hp hq hstart
    exact Ps.Disj p.start p q hp hq start_vx_mem (hstart ▸ start_vx_mem)
  · rintro x ⟨p, hp, rfl⟩
    use p

lemma FinishSet_ncard {Ps : PathEnsemble α β} : Ps.FinishSet.ncard = Ps.Paths.ncard := by
  rw [eq_comm]
  apply Set.ncard_congr (fun p hp ↦ Path.finish p)
  · rintro p hp
    use p
  · rintro p q hp hq hfinish
    exact Ps.Disj p.finish p q hp hq finish_vx_mem (hfinish ▸ finish_vx_mem)
  · rintro x ⟨p, hp, rfl⟩
    use p
...
lemma finite_of_finite_graph {Ps : PathEnsemble α β} {G : Graph α β} (h : G.Finite) (hVd : Ps.ValidOn G) :
    Ps.Paths.Finite := by
...
@[simp]
lemma mem_insert_iff {Ps : PathEnsemble α β} {p q : Path α β} (h : ∀ v ∈ p.vx, v ∉ Ps.VxSet) :
    q ∈ (Ps.insert p h).Paths ↔ q = p ∨ q ∈ Ps.Paths := by
  simp only [insert, union_singleton, Set.mem_insert_iff]

lemma insert_validOn {Ps : PathEnsemble α β} {p : Path α β} (h : ∀ v ∈ p.vx, v ∉ Ps.VxSet)
    (hVd : Ps.ValidOn G) (hpVd : p.ValidOn G) :
    (Ps.insert p h).ValidOn G := by
  rintro q hq
...
@[simp]
lemma insert_ncard {Ps : PathEnsemble α β} {p : Path α β} (h : ∀ v ∈ p.vx, v ∉ Ps.VxSet)
    (hFin : Ps.Paths.Finite) : (Ps.insert p h).Paths.ncard = Ps.Paths.ncard + 1 := by
  simp [insert, VxSet] at h ⊢
  apply Set.ncard_insert_of_not_mem ?_ hFin
  rintro hp
  obtain ⟨a, as, has⟩ := List.ne_nil_iff_exists_cons.mp (vx_ne_nil (w := p.val))
  specialize h a (by simp [has, vx]) p hp
  simp [has, vx] at h

@[simp]
lemma insert_startSet {Ps : PathEnsemble α β} {p : Path α β} (h : ∀ v ∈ p.vx, v ∉ Ps.VxSet) :
    (Ps.insert p h).StartSet = Ps.StartSet ∪ {p.start} := by
  simp only [StartSet, insert, union_singleton]
  exact image_insert_eq

@[simp]
lemma insert_finishSet {Ps : PathEnsemble α β} {p : Path α β} (h : ∀ v ∈ p.vx, v ∉ Ps.VxSet) :
    (Ps.insert p h).FinishSet = Ps.FinishSet ∪ {p.finish} := by
