...
def inf (G H : Graph α β) : Graph α β := Graph.mk' (G.V ∩ H.V)
...
@[simp]
lemma edgeRestrict_eq_self_iff (G : Graph α β) (F : Set β) : G ↾ F = G ↔ G.E ⊆ F := by
  constructor <;> intro h
  · rw [← h]
...
@[simp]
lemma edgeDelete_edgeSet_subset (G : Graph α β) (F : Set β) : (G ＼ F).E ⊆ G.E := by
  simp [edgeDelete_edgeSet]
  exact diff_subset

@[simp]
lemma edgeDelete_edgeSet_ncard_le (G : Graph α β) (F : Set β) [Finite G.E] :
    (G ＼ F).E.ncard ≤ G.E.ncard := ncard_le_ncard <| G.edgeDelete_edgeSet_subset F

@[simp↓]
lemma edgeDelete_edgeSet_ncard_lt_iff (G : Graph α β) (F : Set β) [Finite G.E] :
...
@[simp↓]
lemma edgeDelete_singleton_edgeSet_ncard_lt_iff (G : Graph α β) (e : β) [Finite G.E] :
...
@[simp]
lemma edgeDelete_eq_self_iff (G : Graph α β) (F : Set β) : G ＼ F = G ↔ Disjoint G.E F := by
...
lemma edgeDelete_eq_self (G : Graph α β) (hF : Disjoint G.E F) : G ＼ F = G := by
...
instance instVxSetIsEmptyBot : IsEmpty (⊥ : Graph α β).V := by
  simp

instance instESetIsEmptyBot : IsEmpty (⊥ : Graph α β).E := by
  simp
...
@[simp]
lemma induce_eq_self_iff (G : Graph α β) (X : Set α) : G[X] = G ↔ X = G.V := by
  constructor <;> intro h
  · rw [← h]
    rfl
