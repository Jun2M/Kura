...
def simple_of_acyclic (hGacyc : G.acyclic) : Simple G where
...
omit [DecidableEq V] [DecidableEq W] [Undirected G] in
lemma Emb.acyclic {H : Graph W F} (hHG : H ⊆ᴳ G) : G.acyclic → H.acyclic := by
  intro hGacyclic n
  by_contra! h
...
lemma get_not_conn'_of_acyclic {G : Graph V E} [Undirected G] (hGacyc : G.acyclic) (e : E) :
    ¬ (G{{e}ᶜ}ᴳ.valEs rfl).conn' (G.get e) := by
  rw [conn', ← lift_v12 (by simp only [Subgraph.valEs, conn_comm, implies_true]) e]
  contrapose! hGacyc
  obtain ⟨P, hPstart, hPfinish⟩ := hGacyc.path
  simp only [acyclic, not_forall, not_isEmpty_iff]
  let P' := ((G{{e}ᶜ}ᴳ).toGraph_Emb _ _).Path P
  use P'.length.succ, inferInstance
  refine ⟨?_⟩
  convert Cycle.CycleGraph_Emb G (Cycle.OfPathEndsAdj G P' e ?_ ?_)
  rotate_right 2
  · simp only [Emb.Path_finish, hPfinish,
    Subgraph.toGraph_Emb_fᵥ, Es_val_Sᵥ, Function.comp_apply, Equiv.Set.univ_symm_apply,
    Emb.Path_start, hPstart, P']
    rw [canGo_comm]
    exact canGo_v1_v2 G e
  · simp only [Emb.Path_edges, Subgraph.toGraph_Emb_fₑ, Es_val_Sₑ, Equiv.refl_symm, Equiv.coe_refl,
    CompTriple.comp_eq, List.mem_map, Subtype.exists, Set.mem_compl_iff, Set.mem_singleton_iff,
    exists_and_right, exists_eq_right, not_true_eq_false, IsEmpty.exists_iff, not_false_eq_true, P']
  all_goals simp only [Nat.succ_eq_add_one, Cycle.OfPathEndsAdj, Closed.ofWalkEndAdj, Walk.append_length,
    Walk.some_length, P']

lemma get_not_conn'_iff_acyclic {G : Graph V E} [Undirected G] :
    (∀ e, ¬ (G{{e}ᶜ}ᴳ.valEs rfl).conn' (G.get e)) ↔ G.acyclic := by
  ...
  use S.fₑ (-1)
  rw [Hom.get S]
  let a := (CycleGraph n){{-1}ᶜ}ᴳ
  let b := G{{S.fₑ (-1)}ᶜ}ᴳ
  let c := a.Emb_of_Emb_le S b (by simp only [Subgraph.mapByHom, Fin.neg_one_eq_last', Es_val_Sᵥ,
    Set.image_univ, Es_val_Sₑ, Subgraph.le_iff, Set.subset_univ, Set.subset_compl_singleton_iff,
    Set.mem_image, Set.mem_compl_iff, Set.mem_singleton_iff, Fin.ext_iff', Fin.val_last',
    Emb.fₑinjEq, not_and_self, exists_const, not_false_eq_true, and_self, a, b])
  let d : a.valEs rfl ⊆ᴳ b.valEs rfl :=
    a.toGraph_Isom_toGraph.toEmb.trans (c.trans b.toGraph_Isom_toGraph.toEmb)
  apply d.conn'
  unfold a
  convert ((CycleGraph.toPathGraph n)).symm.conn' (all_conn'_get s(-1, 0))
  simp [Fin.neg_one_eq_last', CycleGraph_get, Fin.last'_add_one_eq_zero, Subgraph.valEs,
    CycleGraph_toPathGraph_symm_fᵥ, Sym2.map_pair_eq, Fin.neg_one_eq_last, Fin.val_last,
    Fin.val_zero, Fin.mk_zero', Sym2.eq, Sym2.rel_iff', Prod.mk.injEq, Fin.ext_iff', Fin.val_last',
    and_self, Prod.swap_prod_mk, true_or, b, a]
...
lemma get_not_conn'_iff_acyclic_of_Es_singleton_compl_acyclic {G : Graph V E} [Undirected G]
    (e : E) (hGe : (G{{e}ᶜ}ᴳ.valEs rfl).acyclic) :
    (¬ (G{{e}ᶜ}ᴳ.valEs rfl).conn' (G.get e)) ↔ G.acyclic := by
  ...
  · obtain ⟨i, rfl⟩ := hS
    ...
    apply d.conn'
    unfold a
    convert ((CycleGraph_Subgraph_singleton_comple_Isom_PathGraph n i)).symm.conn' (all_conn'_get s(-1, 0))
    simp only [CycleGraph_get, Subgraph.valEs,
      CycleGraph_Subgraph_singleton_comple_Isom_PathGraph_symm_fᵥ, Sym2.map_pair_eq,
      Fin.neg_one_eq_last, Fin.cast_zero, zero_add, Sym2.eq, Sym2.rel_iff', Prod.mk.injEq,
      self_eq_add_left, Fin.ext_iff', Fin.val_zero, Prod.swap_prod_mk, Fin.val_one',
      Nat.one_mod_eq_zero_iff, b, a]
    left
    constructor
    · rw [Fin.cast_last_to_last, Fin.last'_add_one_eq_zero]
      rfl
    · rw [add_comm]
...
--     · rintro v w
--       exact G.Es_spanningsubgraph {e}ᶜ |>.toEmb.adj
--     · rintro v w hG
...
--     have hG'Subgr := Es_spanningsubgraph G {e}ᶜ |>.toEmb
...
--           let Sl := PathGraph_Emb_Pathgraph this
--           have hslstart : Sl.fᵥ 0 = _ := PathGraph_Emb_Pathgraph_start this
--           have hslend : Sl.fᵥ _ = _:= PathGraph_Emb_Pathgraph_end this
--           have hnee: ∀ (e_1 : Fin (n - i.succ)), (Sl.trans S).fₑ e_1 ∈ ({e}ᶜ : Set _) := by
--             rintro e'
--             simp only [← hi, Nat.succ_eq_add_one, Emb.trans_fₑ, Function.comp_apply,
--               Set.mem_compl_iff, Set.mem_singleton_iff]
--             refine S.fₑinj.ne ?_
--             simp only [Nat.succ_eq_add_one, PathGraph_Emb_Pathgraph_fₑapply,
...
--           let Sp := PathGraph_Emb_Pathgraph this
--           have hspstart : Sp.fᵥ 0 = 0 := PathGraph_Emb_Pathgraph_start this
--           have hspend : Sp.fᵥ _ = _ := PathGraph_Emb_Pathgraph_end this
--           have hnee: ∀ (e_1 : Fin i), (Sp.trans S).fₑ e_1 ∈ ({e}ᶜ : Set E) := by
--             rintro e'
--             simp only [← hi, Nat.succ_eq_add_one, Emb.trans_fₑ, Function.comp_apply,
--               Set.mem_compl_iff, Set.mem_singleton_iff]
--             refine S.fₑinj.ne ?_
--             rw [PathGraph_Emb_Pathgraph_fₑapply, ← Fin.val_inj.ne]
...
--         apply conn.OfPathEmb S' <;> simp only [hfᵥ, ← hs, Fin.natCast_eq_last, ← ht]
...
--   have hGBundir : GB.Undirected := G.EVSubgraphSpanningEmb S B sorry |>.Undirected'
--   have hGAundir : GA.Undirected := G.EVSubgraphSpanningEmb S A sorry |>.Undirected'
