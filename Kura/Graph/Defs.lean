...
lemma toGraph_Isom_toGraph_fᵥ (S : Subgraph G) {V' V'' E' E'' : Type*} (hSv1 : S.Sᵥ.Elem ≃ V')
    (hSv2 : S.Sᵥ.Elem ≃ V'') (hSe1 : S.Sₑ.Elem ≃ E') (hSe2 : S.Sₑ.Elem ≃ E'') :
    (S.toGraph_Isom_toGraph hSv1 hSv2 hSe1 hSe2).fᵥ = hSv1.symm.trans hSv2 := rfl

@[simp]
lemma toGraph_Isom_toGraph_fₑ (S : Subgraph G) {V' V'' E' E'' : Type*} (hSv1 : S.Sᵥ.Elem ≃ V')
    (hSv2 : S.Sᵥ.Elem ≃ V'') (hSe1 : S.Sₑ.Elem ≃ E') (hSe2 : S.Sₑ.Elem ≃ E'') :
    (S.toGraph_Isom_toGraph hSv1 hSv2 hSe1 hSe2).fₑ = hSe1.symm.trans hSe2 := rfl

@[simp]
lemma toGraph_Isom_toGraph_symm_fᵥ (S : Subgraph G) {V' V'' E' E'' : Type*} (hSv1 : S.Sᵥ.Elem ≃ V')
    (hSv2 : S.Sᵥ.Elem ≃ V'') (hSe1 : S.Sₑ.Elem ≃ E') (hSe2 : S.Sₑ.Elem ≃ E'') :
    (S.toGraph_Isom_toGraph hSv1 hSv2 hSe1 hSe2).symm.fᵥ = hSv2.symm.trans hSv1 := by
  ext1 v
  apply_fun (S.toGraph_Isom_toGraph hSv1 hSv2 hSe1 hSe2).fᵥ
  simp only [Isom.fᵥ_symm_fᵥ, Equiv.trans_apply, toGraph_Isom_toGraph_fᵥ, Equiv.symm_apply_apply,
    Equiv.apply_symm_apply]
  exact (S.toGraph_Isom_toGraph hSv1 hSv2 hSe1 hSe2).fᵥinj

@[simp]
lemma toGraph_Isom_toGraph_symm_fₑ (S : Subgraph G) {V' V'' E' E'' : Type*} (hSv1 : S.Sᵥ.Elem ≃ V')
    (hSv2 : S.Sᵥ.Elem ≃ V'') (hSe1 : S.Sₑ.Elem ≃ E') (hSe2 : S.Sₑ.Elem ≃ E'') :
    (S.toGraph_Isom_toGraph hSv1 hSv2 hSe1 hSe2).symm.fₑ = hSe2.symm.trans hSe1 := by
  ext1 e
  apply_fun (S.toGraph_Isom_toGraph hSv1 hSv2 hSe1 hSe2).fₑ
  simp only [Isom.fₑ_symm_fₑ, Equiv.trans_apply, toGraph_Isom_toGraph_fₑ, Equiv.symm_apply_apply,
    Equiv.apply_symm_apply]
  exact (S.toGraph_Isom_toGraph hSv1 hSv2 hSe1 hSe2).fₑinj
...
def Emb_of_le {S T : Subgraph G} (hST : S ≤ T) {V' V'' E' E'' : Type*}
    (hSv1 : S.Sᵥ.Elem ≃ V') (hSe1 : S.Sₑ.Elem ≃ E') (hSv2 : T.Sᵥ.Elem ≃ V'')
    (hSe2 : T.Sₑ.Elem ≃ E'') : (S.toGraph hSv1 hSe1) ⊆ᴳ (T.toGraph hSv2 hSe2) where
  fᵥ v := hSv2 <| Subtype.impEmbedding _ _ hST.1 (hSv1.symm v)
  fₑ e := hSe2 <| Subtype.impEmbedding _ _ hST.2 (hSe1.symm e)
  inc e := by
    simp only [toGraph, Subtype.impEmbedding, Function.Embedding.coeFn_mk, Equiv.symm_apply_apply,
      map_pmap]
    apply pmap_congrFun
...
def Hom_of_mapByHom (A : G.Hom H) (S : G.Subgraph) (T : H.Subgraph) (hT : T = mapByHom A S)
    {V' V'' E' E'' : Type*} (hSv1 : S.Sᵥ.Elem ≃ V') (hSe1 : S.Sₑ.Elem ≃ E') (hSv2 : T.Sᵥ.Elem ≃ V'')
    (hSe2 : T.Sₑ.Elem ≃ E'') :
    (S.toGraph hSv1 hSe1).Hom (T.toGraph hSv2 hSe2) where
  fᵥ v := hSv2 (⟨A.fᵥ (hSv1.symm v), by
    subst T
    simp only [mapByHom, Set.mem_image]
    use hSv1.symm v
    simp only [Subtype.coe_prop, and_self]⟩)
  fₑ e := hSe2 (⟨A.fₑ (hSe1.symm e), by
    subst T
    simp only [mapByHom, Set.mem_image]
    use hSe1.symm e
    simp only [Subtype.coe_prop, and_self]⟩)
  inc e := by
    simp only [toGraph, mapByHom, Equiv.symm_apply_apply, A.inc (hSe1.symm e)]
    rw [map_pmap, pmap_map]
    ...
    · subst T
      ...
      use v
      simp only [and_true]
      exact S.spec (hSe1.symm e) (hSe1.symm e).prop v hv

omit [DecidableEq W] in @[simp]
lemma Hom_of_mapByHom_fᵥ (A : G.Hom H) (S : G.Subgraph) (T : H.Subgraph) (hT : T = mapByHom A S)
    {V' V'' E' E'' : Type*} (hSv1 : S.Sᵥ.Elem ≃ V') (hSe1 : S.Sₑ.Elem ≃ E') (hSv2 : T.Sᵥ.Elem ≃ V'')
    (hSe2 : T.Sₑ.Elem ≃ E'') :
    (Hom_of_mapByHom A S T hT hSv1 hSe1 hSv2 hSe2 ).fᵥ = fun v ↦ hSv2 (⟨A.fᵥ (hSv1.symm v), by
      subst T
      simp only [mapByHom, Set.mem_image]
      use hSv1.symm v
...
lemma Hom_of_mapByHom_fₑ (A : G.Hom H) (S : G.Subgraph) (T : H.Subgraph) (hT : T = mapByHom A S)
    {V' V'' E' E'' : Type*} (hSv1 : S.Sᵥ.Elem ≃ V') (hSe1 : S.Sₑ.Elem ≃ E') (hSv2 : T.Sᵥ.Elem ≃ V'')
    (hSe2 : T.Sₑ.Elem ≃ E'') :
    (Hom_of_mapByHom A S T hT hSv1 hSe1 hSv2 hSe2).fₑ = fun e ↦ hSe2 (⟨A.fₑ (hSe1.symm e), by
      subst T
      simp only [mapByHom, Set.mem_image]
      use hSe1.symm e
      simp only [Subtype.coe_prop, and_self]⟩) := rfl
...
def Hom_of_Hom_le (A : G.Hom H) (S : G.Subgraph) (T : H.Subgraph) (hT : mapByHom A S ≤ T)
    {V' V'' E' E'' : Type*} (hSv1 : S.Sᵥ.Elem ≃ V') (hSe1 : S.Sₑ.Elem ≃ E') (hSv2 : T.Sᵥ.Elem ≃ V'')
    (hSe2 : T.Sₑ.Elem ≃ E'') :
    (S.toGraph hSv1 hSe1).Hom (T.toGraph hSv2 hSe2) := by
  refine (?_ : (S.toGraph hSv1 hSe1).Hom (mapByHom A S)).trans (Emb_of_le hT _ _ hSv2 hSe2).toHom
  apply Hom_of_mapByHom
  rfl

def Emb_of_Emb_le (A : G ⊆ᴳ H) (S : G.Subgraph) (T : H.Subgraph) (hT : mapByHom A.toHom S ≤ T) :
    S.val ⊆ᴳ T.val where
  toHom := Hom_of_Hom_le A S T hT
  fᵥinj v w h := by
    ext
...
def Emb_of_mapByHom (A : G ⊆ᴳ H) (S : G.Subgraph) (T : H.Subgraph) {V' V'' E' E'' : Type*}
    (hSv1 : S.Sᵥ.Elem ≃ V') (hSe1 : S.Sₑ.Elem ≃ E') (hSv2 : T.Sᵥ.Elem ≃ V'')
    (hSe2 : T.Sₑ.Elem ≃ E'') (hT : T = mapByHom A.toHom S) :
    (S.toGraph hSv1 hSe1) ⊆ᴳ T.toGraph hSv2 hSe2 where
  toHom := Hom_of_mapByHom A.toHom S T hSv1 hSe1 hSv2 hSe2 hT
  fᵥinj v w h := by simpa only [Hom_of_mapByHom_fᵥ, EmbeddingLike.apply_eq_iff_eq,
    Subtype.mk.injEq, Emb.fᵥinjEq, Subtype.val_injective.eq_iff, hSv1.symm.injective.eq_iff] using h
  fₑinj e f h := by simpa only [Hom_of_mapByHom_fₑ, EmbeddingLike.apply_eq_iff_eq,
    Subtype.mk.injEq, Emb.fₑinjEq, Subtype.val_injective.eq_iff, hSe1.symm.injective.eq_iff] using h
...
def Isom_of_equivOfIsom (A : G ≃ᴳ H) (S : G.Subgraph) (T : H.Subgraph) {V' V'' E' E'' : Type*}
    (hSv1 : S.Sᵥ.Elem ≃ V') (hSe1 : S.Sₑ.Elem ≃ E') (hSv2 : T.Sᵥ.Elem ≃ V'')
    (hSe2 : T.Sₑ.Elem ≃ E'') (hT : T = mapByHom A.toHom S) :
    S.toGraph hSv1 hSe1 ≃ᴳ T.toGraph hSv2 hSe2 where
  toEmb := Emb_of_mapByHom A.toEmb S T hSv1 hSe1 hSv2 hSe2 hT
  fᵥsurj w := by
    subst T
    simp only [equivOfIsom, mapByHom, Equiv.coe_fn_mk, Emb_of_mapByHom, Hom_of_mapByHom_fᵥ]
    use hSv1 ⟨A.symm.fᵥ (hSv2.symm w), ?_⟩
    simp only [Equiv.symm_apply_apply, Isom.fᵥ_symm_fᵥ, Subtype.coe_eta]
    exact Equiv.apply_symm_apply hSv2 w
    · have := (hSv2.symm w).prop
      simp only [mapByHom, Set.mem_image] at this ⊢
      obtain ⟨v, hv, h⟩ := this
      rwa [← h, Isom.symm_fᵥ_fᵥ]
  fₑsurj f := by
    subst T
    simp only [equivOfIsom, mapByHom, Equiv.coe_fn_mk, Emb_of_mapByHom, Hom_of_mapByHom_fₑ]
    use hSe1 ⟨A.symm.fₑ (hSe2.symm f), ?_⟩
    simp only [Equiv.symm_apply_apply, Isom.fₑ_symm_fₑ, Subtype.coe_eta]
    exact Equiv.apply_symm_apply hSe2 f
    · have := (hSe2.symm f).prop
      simp only [mapByHom, Set.mem_image] at this ⊢
      obtain ⟨e, he, h⟩ := this
