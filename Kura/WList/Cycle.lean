...
lemma IsClosed.rotate_vertexSet (hw : w.IsClosed) (n) : V(w.rotate n) = V(w) := by
  simp_rw [← (hw.rotate _).vertexSet_tail, WList.vertexSet, ← mem_vx, rotate_vx_tail, List.mem_rotate, mem_vx]
...
lemma IsClosed.rotate_length (hw : w.IsClosed) : w.rotate w.length = w := by
  ...
  | cons u e w =>
    rw [cons_length, rotate_cons_succ, cons_vx,
      ...
      ← tail_vx_of_nonempty (by simp), rotate_vx_tail, vx_head, rotate_first _ _ (by simp),
