...
@[simp] lemma Nil.vx_eq_first (h : w.Nil) : w.vx = [w.first] := by
  cases h with rfl

@[simp] lemma Nil.vx_eq_last (h : w.Nil) : w.vx = [w.last] := by
  cases h with rfl

@[simp] lemma Nil.edge_eq_nil (h : w.Nil) : w.edge = [] := by
  cases h with rfl

@[simp] lemma Nil.not_mem_edge (hN : w.Nil) : e ∉ w.edge := by
  rw [hN.edge_eq_nil]
  simp
