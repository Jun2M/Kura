...
@[simp]
lemma mem_vertexSet_iff : x ∈ w.vertexSet ↔ x ∈ w := Iff.rfl
...
instance instNilDecidable : Decidable (w.Nil) := by
...
@[simp] lemma firstEdge_cons : (cons x e w).firstEdge (cons_nonempty _ _ _) = e := rfl

lemma firstEdge_mem_edge (hw : w.Nonempty) : w.firstEdge hw ∈ w.edge := by
  cases w with simp at hw ⊢
...
protected lemma IsLink.of_cons (hw : (WList.cons u f w).IsLink e x y) (hef : e ≠ f) : w.IsLink e x y := by
...
lemma IsLink.vx_mem_left (h : w.IsLink e x y) : x ∈ w := by
  induction h with simp_all

lemma IsLink.vx_mem_right (h : w.IsLink e x y) : y ∈ w :=
  h.symm.vx_mem_left
...
-- namespace WList.vertexSetalidIn
