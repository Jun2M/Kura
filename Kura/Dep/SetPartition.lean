...
@[simps] def ofIndependent {u : Set α} (hs : _root_.sSupIndep u) (hbot : ⊥ ∉ u) :
...
@[simp] lemma mem_ofIndependent_iff {u : Set α} (hu : _root_.sSupIndep u)
    (h : ⊥ ∉ u) {a : α} : a ∈ ofIndependent hu h ↔ a ∈ u := Iff.rfl

/-- A `sSupIndep` collection gives a partition of its supremum by removing `⊥`. -/
def ofIndependent' {u : Set α} (hs : _root_.sSupIndep u) : Partition (sSup u) :=
  (ofIndependent (hs.mono (diff_subset (t := {⊥}))) (fun h ↦ h.2 rfl)).copy (by simp)

@[simp] lemma mem_ofIndependent'_iff {u : Set α} (hu : _root_.sSupIndep u) {a : α} :
...
@[simps] def indiscrete (s : α) (hs : s ≠ ⊥) : Partition s where
  parts := {s}
  sSupIndep' := by simp [_root_.sSupIndep]
...
instance {s : α} : OrderTop (Partition s) where
  top := (ofIndependent' (sSupIndep_singleton s)).copy sSup_singleton
  le_top := by
    obtain (rfl | hs) := eq_or_ne s ⊥
    · simp
    rintro P x hx
    refine ⟨s, ?_, P.le_of_mem hx⟩
    change s ∈ Partition.copy ..
    rw [mem_copy_iff, mem_ofIndependent'_iff]
    exact ⟨rfl, hs⟩

@[simp] lemma mem_top_iff {a s : α} : a ∈ (⊤ : Partition s) ↔ a = s ∧ a ≠ ⊥ := by
  change a ∈ Partition.copy _ _ ↔ _
  rw [mem_copy_iff, mem_ofIndependent'_iff, mem_singleton_iff]
...
@[simps] protected def bind (P : Partition s) (Qs : ∀ a ∈ P, Partition a) : Partition s where
  ...
  sSupIndep' := by
    ...
    have hdj2 := (P.sSupIndep haP).mono_left <| (Qs a haP).le_of_mem hba
    refine (hdj1.sup_right hdj2).mono_right ?_
    simp only [mem_iUnion, SetLike.mem_coe, Subtype.exists, not_exists, ge_iff_le,
...
@[simps] def ofPairwiseDisjoint {p : Set (Set α)} (h : p.PairwiseDisjoint id) (h_empty : ∅ ∉ p):
    Partition (⋃₀ p) where
  parts := p
  sSupIndep' := PairwiseDisjoint.sSupIndep h
  bot_not_mem' := h_empty
  sSup_eq' := rfl
...
@[simps] def ofPairwiseDisjoint' {s : Set α} {parts : Set (Set α)}
  ...
  (forall_nonempty : ∀ s ∈ parts, s.Nonempty) (eq_sUnion : s = ⋃₀ parts) :
    Partition s where
  parts := parts
  sSupIndep' := pairwiseDisjoint.sSupIndep
...
@[simp] lemma mem_ofRel'_iff {t : Set α} (hs : s = {x | r x x}):
  ...
  subst hs
  simp [ofRel', mem_copy_iff, mem_ofRel_iff]
...
@[simp] lemma rel_congr (P : Partition s) (hst : s = t) : (P.copy hst).Rel = P.Rel := rfl
