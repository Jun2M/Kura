...
inductive Prewalk (α β : Type*) where
| nil (u : α) : Prewalk α β
...
def vx : Walk α β → List α := fun w => w.prewalk.vx
...
def cons (x : α) (e : β) (w : Walk α β)
  ...
  ⟨Prewalk.cons x e w.prewalk, fun f ↦ by
    by_cases h : f = e
    · subst f
      simpa [Prewalk.edgeSigniture]
    · simp [Prewalk.edgeSigniture, h]
      exact w.prop f⟩
...
variable (h : (insert s(x, w.first) (w.prewalk.edgeSigniture e)).ncard ≤ 1)
namespace Walk

/- Properties of nil -/

@[simp] lemma nil_not_nonempty : ¬ (nil x : Walk α β).Nonempty := by simp [Nonempty, nil]
...
@[simp] lemma cons_nonempty : (cons x e w h).Nonempty := by simp [Nonempty, cons]

@[simp] lemma cons_first : (cons x e w h).first = x := rfl

@[simp] lemma cons_last : (cons x e w h).last = w.last := rfl

@[simp] lemma cons_vx : (cons x e w h).vx = x :: w.vx := rfl

@[simp] lemma mem_cons_iff : u ∈ (cons x e w h) ↔ u = x ∨ u ∈ w := by
  simp [← mem_notation, cons, Prewalk.vx, vx]

@[simp] lemma cons_vxSet : (cons x e w h).vxSet = {x} ∪ w.vxSet := by
  simp only [vxSet, mem_cons_iff, singleton_union]
  rfl

@[simp↓] lemma cons_vxSet_subset : (cons x e w h).vxSet ⊆ U ↔ x ∈ U ∧ w.vxSet ⊆ U := by
  simp [insert_subset_iff]

@[simp] lemma cons_edge : (cons x e w h).edge = e :: w.edge := rfl

@[simp] lemma cons_edgeSet : (cons x e w h).edgeSet = {e} ∪ w.edgeSet := by
  simp only [edgeSet, cons_edge, mem_cons, singleton_union]
  rfl

@[simp] lemma cons_length : (cons x e w h).length = w.length + 1 := rfl

@[simp] lemma cons_validIn : (cons x e w h).ValidIn G ↔ G.Inc₂ e x w.first ∧ w.ValidIn G :=
  ⟨id, id⟩

@[simp]
lemma ValidIn.cons (hw : (cons x e w h).ValidIn G) : w.ValidIn G := by
  rw [cons_validIn] at hw
  exact hw.2

@[simp] lemma cons_isWalkFrom : G.IsWalkFrom S T (cons x e w h) ↔
...
@[simp]
lemma cons_vx_nodup (h : (cons x e w h).vx.Nodup) : w.vx.Nodup := by
  simp only [cons_vx, nodup_cons] at h
  exact h.2

@[simp] lemma cons_isTrail : G.IsTrail (cons x e w h) ↔
...
@[simp] lemma cons_isPath : G.IsPath (cons x e w h) ↔ G.IsPath w ∧ G.Inc₂ e x w.first ∧ x ∉ w := by
  constructor
...
@[simp]
lemma cons_isTrailFrom : G.IsTrailFrom S T (cons x e w h) ↔
...
@[simp]
lemma cons_isPathFrom : G.IsPathFrom S T (cons x e w h) ↔
...
@[simp]
lemma IsTrail.cons (h : G.IsTrail (Walk.cons x e w h)) : G.IsTrail w := by
  rw [cons_isTrail] at h
  exact h.1

@[simp]
lemma IsPath.cons (h : G.IsPath (cons x e w h)) : G.IsPath w := by
  rw [cons_isPath] at h
  exact h.1
...
@[simp] lemma first_mem : w.first ∈ w := by
  obtain ⟨w, p⟩ := w
  match w with
  | .nil x => simp [first, ← mem_notation, vx]
  | .cons x e w => simp [first, ← mem_notation, vx]
