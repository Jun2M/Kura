...
@[simp]
lemma walk_vertexSet (h : G.Inc₂ e u v): V(h.walk) = {u, v} := by
...
@[simp]
lemma walk_edgeSet (h : G.Inc₂ e u v): E(h.walk) = {e} := by
  simp [WList.edgeSet]
...
lemma IsWalk.induce (hw : G.IsWalk w) (hX : V(w) ⊆ X) : G[X].IsWalk w := by
  ...
  | nil => simp_all
  | @cons x e w hw h ih =>
    simp_all only [cons_vertexSet, insert_subset_iff, cons_isWalk_iff, induce_inc₂, true_and,
...
lemma isWalk_induce_iff' (hw : w.Nonempty) : G[X].IsWalk w ↔ G.IsWalk w ∧ V(w) ⊆ X := by
  ...
  | cons u e w ih => cases w with
    | nil v =>
      simp only [cons_isWalk_iff, nil_first, induce_inc₂, nil_isWalk_iff, induce_vertexSet] at h ⊢
...
lemma IsWalk.le_of_edgeSet_subset (hw₁ : G.IsWalk w₁) (hne : w₁.Nonempty) (hw₂ : G.IsWalk w₂)
  ...
  rw [toGraph_vertexSet, mem_vertexSet_iff, hne.mem_iff_exists_inc₂] at hxV
  obtain ⟨y, e, h⟩ := hxV
