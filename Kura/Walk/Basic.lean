...
lemma IsWalk.vx_mem_of_mem (h : G.IsWalk w) (hmem : x ∈ w) : x ∈ V(G) := by
...
lemma IsWalk.isLink_iff_isLink_of_mem (h : G.IsWalk w) (hew : e ∈ w.edge) :
  ...
  obtain ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ := h'.eq_and_eq_or_eq_and_eq_of_isLink (h.isLink_of_isLink hx'y')
  · assumption
  exact hx'y'.symm
...
@[simp] lemma walk_nonempty (h : G.IsLink e u v): h.walk.Nonempty := by simp [walk]

@[simp]
lemma walk_vx (h : G.IsLink e u v): h.walk.vx = [u, v] := rfl
...
lemma length_eq_one_iff : w.length = 1 ∧ G.IsWalk w ↔ ∃ x e y h, w = (h : G.IsLink e x y).walk := by
  ...
  exact ⟨x, e, y, hVd.1, by simp [IsLink.walk]⟩
...
lemma isWalk_induce_iff' (hw : w.Nonempty) : G[X].IsWalk w ↔ G.IsWalk w ∧ V(w) ⊆ X := by
  ...
  | cons u e w ih => cases w with
    | nil v =>
      simp only [cons_isWalk_iff, nil_first, induce_isLink, nil_isWalk_iff, induce_vertexSet] at h ⊢
...
@[simp]
lemma isWalk_vxDelete_iff : (G - X).IsWalk w ↔ G.IsWalk w ∧ Disjoint V(w) X := by
...
--   nodup := by
--     simp only [Set.subset_singleton_iff, Set.mem_inter_iff, mem_vertexSet_iff, and_imp, append_vx,
--       nodup_append, h₁.nodup.sublist w₁.vx.dropLast_sublist, h₂.nodup, true_and] at hvertexSet ⊢
--     rintro x hx₁ hx₂
...
lemma WellFormed.reverse_toGraph (h : w.WellFormed) : w.reverse.toGraph = w.toGraph :=
  Graph.ext (by simp) fun e x y ↦ by rw [h.toGraph_isLink, h.reverse.toGraph_isLink, isLink_reverse_iff]
...
lemma IsWalk.toGraph_eq_induce_restrict (h : G.IsWalk w) : w.toGraph = G[V(w)] ↾ E(w) := by
  ...
  | cons u e w ih =>
    ...
    · refine (edgeRestrict_le ..).trans (induce_le ?_)
      simp [insert_subset_iff, h.1.vx_mem_left, h.2.vertexSet_subset]
