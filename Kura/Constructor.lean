...
@[simps!]
def Simplify (G : Graph α β) : Graph G.V (Sym2 G.V) :=
  G.toSimpleGraph.toGraph

instance : IsSimple (Simplify G) where
  loopless x := by simp only [Simplify, SimpleGraph.toGraph_adj, SimpleGraph.irrefl,
    not_false_eq_true]
  no_multi_edges e f he hf h := by
    induction' e with x y
    simp only [Simplify_edgeSet, SimpleGraph.mem_edgeSet, toSimpleGraph_adj] at he
    simp only [Simplify, toSym2_eq_toSym2_iff', SimpleGraph.toGraph_inc₂, toSimpleGraph_adj, ne_eq,
      and_congr_left_iff, and_imp] at h
    specialize h x y he.1 he.2
    tauto

@[simp]
lemma simplify_adj : (Simplify G).Adj x y ↔ x ≠ y ∧ G.Adj x y := by
  simp only [Simplify, SimpleGraph.toGraph_adj, toSimpleGraph_adj]

#print Simplify_vxSet
